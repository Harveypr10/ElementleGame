...
- **Pro Subscription System**: Three-tier Pro subscription (Bronze $3.99/mo, Silver $6.99/mo, Gold $9.99/mo) with ad removal and category selection features. Components: `GoProButton` (crown icon in GameSelectionPage header), `ProSubscriptionDialog` (tier selection modal), `CategorySelectionScreen` (min 3 category picks), and `useSubscription` hook. Schema tables: `user_subscriptions` and `user_pro_categories` (see SQL comments in shared/schema.ts for Supabase migration commands).
...
- **Guest Restrictions**: Guest users (unauthenticated) cannot swipe to Personal (Local) mode or access Archive. Swipe attempts trigger snap-back animation and restriction popups with hamster image and Register/Login buttons. AuthPage styling with light blue accent (#7DAAE8) for registration prompts.
