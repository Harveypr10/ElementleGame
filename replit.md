...
- **Preload & Cache System**: `PreloadProvider` for critical assets and data (settings, profile, stats, attempts, puzzles, user categories, categories list, subscription, regions), prioritizing cache-first rendering. Subscription data includes autoRenew state and requires auth token for prefetching. User pro-categories require auth token for prefetching. Categories list is cached for 1 hour. Date format settings are instantly cached in LocalStorage. Profile and regions are prefetched for instant Account Info page display.
...
- **Loading Spinner System**: React-controlled spinner via `SpinnerProvider` with hamster wheel Lottie animation. Features 150ms delay before showing to avoid flash on fast loads, 300ms fade in/out animation, minimum 1 second display time, and "Loading" text in medium grey below the animation. Controls visibility per-page/per-route without interfering with framer-motion transitions. Animation at `/assets/hamster.json`.
  - **Timeout/Retry Pattern**: `useSpinnerWithTimeout` hook provides automatic retry after 4 seconds, timeout after 8 seconds with error navigation back to GameSelectionPage.
  - **Data Validation**: Spinners validate that data has meaningful content (not just truthy values) before completing:
    - Profile must have `id` or `email` to be considered valid
    - Regions must be a non-empty array to be considered valid
  - **Components Using Timeout Pattern**: CategorySelectionScreen, AccountInfoPage, PlayPage (game loading), and Home.tsx (auth/puzzle loading) all use the timeout/retry pattern.
