...
- **Intro Screen (New Games)**: `IntroScreen` component displays when entering a new game with no guesses yet. Shows: Historian-Hamster-Blue.svg image, puzzle date formatted per user preference (DD/MM/YY → "25th November 2025", MM/DD/YY → "November 25, 2025"), clue prompt ("On what date in history did this event occur?" when clues enabled, or "Take on the challenge of guessing a date in history!" when disabled) with event title, and a Play button colored to match the Global (#7DAAE8) or Local (#66becb) mode. Light grey background (bg-gray-100 dark:bg-gray-900) with fade-in. Games with any guesses or completed games go directly to the game screen.
