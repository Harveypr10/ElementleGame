...
- **Retry Mechanism for Missing Data**: Both `GameSelectionPage` and `Home.tsx` include retry mechanisms that automatically refetch puzzle data when it's missing for authenticated users. This handles the race condition where realtime events may be missed during initial signup (when the subscription isn't ready before the Worker allocates questions). Retries up to 5 times with 2-second intervals.
