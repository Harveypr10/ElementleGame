-- ============================================================================
-- TESTERS LEAGUE BACKFILL: 2025-10-01 → 2026-03-08  (yesterday)
--
-- This script:
--   1. Clears existing snapshot data for the Testers league in the date range
...
--      completed month (Oct–Feb) and year (2025)
...
-- ─── Step A: Clear existing backfill data for Testers League ────────────────
...
BEGIN
    ...
    WHERE league_id = v_league_id
      AND snapshot_date BETWEEN '2025-10-01' AND '2026-03-08';

    -- Clear user stats snapshots (for users in Testers league only)
    ...
    -- Clear end-of-period snapshots for Testers league
    DELETE FROM public.league_standings_snapshot
    WHERE league_id = v_league_id
      AND period_label IN ('2025-10','2025-11','2025-12','2026-01','2026-02','2025');

    -- Clear existing awards for Testers league in these periods
    DELETE FROM public.league_awards
    WHERE league_id = v_league_id
      AND period_label IN ('2025-10','2025-11','2025-12','2026-01','2026-02','2025');

    RAISE NOTICE 'Cleared existing data for Testers League %', v_league_id;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Starting backfill for Testers League %, min_games = %', v_league_id, v_min_games;

    -- Loop through each day from 2025-10-01 to 2026-03-08
    FOR snap_date IN SELECT generate_series('2025-10-01'::date, '2026-03-08'::date, '1 day'::interval)::date
    LOOP
        ...
        -- Phase 1: Calculate each Testers league member's stats as of snap_date
        FOR v_rec IN
        ...
        -- Phase 2: Rank Testers league members (only users with >= min_games)
...
-- Frozen standings used to display medals for completed months.
...
BEGIN
    ...
    -- Process each completed month + annual 2025
    FOR v_period IN
        SELECT * FROM (VALUES
            ('2025-10', '2025-10-31'::date, 'mtd'),
    ...
    LOOP
        ...
        FROM (
            ...
            WHERE dr.league_id = v_league_id
...
-- ─── Step D: Award trophies (gold/silver/bronze) for top 3 ─────────────────
...
WHERE league_id = 'be94b0e7-639f-436b-a71a-d8e2a8f2ee95'
  AND timeframe = 'mtd'
GROUP BY 1, 2
ORDER BY 1, 2;
...
WHERE la.league_id = 'be94b0e7-639f-436b-a71a-d8e2a8f2ee95'
  AND la.period_label IN ('2025-10','2025-11','2025-12','2026-01','2026-02','2025')
