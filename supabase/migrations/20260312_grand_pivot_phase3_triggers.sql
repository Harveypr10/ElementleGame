-- ============================================================
-- Grand Pivot: Phase 3 Step 4 — Rewrite Split Triggers
-- Run AFTER Steps 1-3 in the specs migration.
-- ============================================================
-- Changes from the original triggers:
--   1. Category specs: region='GLOBAL' (was 'UK')
--   2. Location splits: dynamic region/location from covering spec
--   3. UK locations: match by populated_place_id (osgb*)
--   4. US locations: match by populated_place_id (US-*)
--   5. ROW locations: match by event_origin as region, location IS NULL
--   6. Category inserts also perform 1-Way Split on matching location spec
--   7. 'WW' event_origin fans out to ALL active location specs
...
begin
  if new.question_kind = 'location' then
    ...
    -- ========== CATEGORY SPLIT ==========
    -- Split GLOBAL category specs + 1-Way location spec split
    ...
    loop
      ...
      -- Find covering GLOBAL category spec
      ...
      if q_date <= s_date + make_interval(days => near_threshold) then
        ...
        if q_date < e_date then
          insert into available_question_spec
            (start_date, end_date, region, category_id, location, active, created_at, updated_at)
          values
            (q_date + interval '1 day', e_date, 'GLOBAL', cat, null, true, now(), now())
          returning id into new_spec2_id;
        end if;
      elsif q_date >= e_date - make_interval(days => near_threshold) then
        new_spec2_id := null;
        if q_date > s_date then
          insert into available_question_spec
            (start_date, end_date, region, category_id, location, active, created_at, updated_at)
          values
            (s_date, q_date - interval '1 day', 'GLOBAL', cat, null, true, now(), now())
          returning id into new_spec1_id;
        end if;
      else
        if q_date > s_date then
...
-- Uses GLOBAL region + 1-Way location split.
