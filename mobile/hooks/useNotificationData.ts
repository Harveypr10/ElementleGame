...
export function useNotificationData() {
    ...
    const hydrate = useCallback(async (): Promise<NotificationHydratedData | null> => {
        ...
        try {
            ...
            // The drip campaign target dates (relative to today)
            ...
            const [regionStatsRes, userStatsRes] = await Promise.all([
                supabase.from('user_stats_region').select('*').eq('user_id', user.id).maybeSingle(),
                supabase.from('user_stats_user').select('*').eq('user_id', user.id).maybeSingle(),
            ]);
